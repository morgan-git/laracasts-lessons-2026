    ...
    <form method="POST" action="/ideas/{{ $idea->id }}">
     ...
     @method('PATCH')
      ...
      <div class="col-span-full">
          ...
          <div class="mt-2">
            <textarea id="about" name="idea" rows="3" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6">{{ $idea->description }}</textarea>
          </div>
        </div>
